...
You can list all clients on the *Client List*. This is useful when your *Client List* has been filtered and you want to view all clients again. When using `list-c`, there will be no difference to the *Client List* if it is already showing all clients.
...
This section shows the parameters and the important things to note about them.
...
Imagine that you want to schedule Alex Yeoh with the following slot: +

* Monday 11:00am to 12:00pm +

and also schedule two slots for Bernice Yu. First, let's look at how we can assign one schedule slot to Alex Yeoh.
...
+
. Now that you have successfully schedule for Alex Yeoh, let's move on to add the two schedules to Bernice Yu: Monday 8:00am to 10:00pm and Tuesday 4:00pm to 6:00pm.
You can do that by adding both of these timings into the `schedule` command following the same format as shown below.
+
...
=== Exercise Commands
The table below shows the commands that manipulate the details of the exercises done by a client.
